...
def initialize() {
	...
	def devs = settings.devices.collect { dni ->
		...
		if(!d) {
			...
			d = addChildDevice(app.namespace, getChildName(), dni, null, ["label":"${devlist[dni]} AWAIR" ?: "AWAIR"])
...
def authPage() {
    ...
    if(!oauthTokenProvided) {
        ...
        def devices = makeJSONRequest()
        def devs = [:]
        devs += makeJSONRequest()
		atomicState.devices = devs
        log.debug "available devices list: ${devs}"
        return dynamicPage(name: "auth", title: "Select Your Device", nextPage: "", uninstall:uninstallAllowed) {
            section("") {
...
def pollChildren(){
		...
		settings.devices.each {dev ->
            ...
            def Params = [
                ...
                path: "v1/users/self/devices/${deviceid}",
                headers: ['Authorization': "Bearer ${atomicState.authToken}"],
                query: [
                    apikey: APIKey(),
